...
describe('Tag e2e test', () => {
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/author/api/tags+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/author/api/tags').as('postEntityRequest');
    cy.intercept('DELETE', '/services/author/api/tags/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (tag) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/author/api/tags/${tag.id}`,
      }).then(() => {
        tag = undefined;
      });
    }
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/services/author/api/tags?page=0&size=20>; rel="last",<http://localhost/services/author/api/tags?page=0&size=20>; rel="first"',
              },
              body: [tag],
            }
