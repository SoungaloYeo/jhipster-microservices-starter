...
type RestOf<T extends IBook | NewBook> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class BookService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/books', 'author');
  ...
  protected convertDateFromServer(restBook: RestBook): IBook {
    return {
      ...restBook,
      date: restBook.date ? dayjs(restBook.date) : undefined,
    };
  }
